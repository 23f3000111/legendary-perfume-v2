/**
 * Supabase's own certificate authority.
 *
 * Supabase does not use a public CA for Postgres. The pooler presents a chain
 * signed by "Supabase Root 2021 CA", which is not in Node's trust store, so a
 * connection verified against the public roots fails with
 * SELF_SIGNED_CERT_IN_CHAIN. The usual advice on the internet is to answer that
 * with `rejectUnauthorized: false`, which does not fix the trust problem, it
 * discards it, and leaves the database connection open to interception.
 *
 * Pinning Supabase's roots here is both secure and stricter than the public CA
 * set would have been: this connection now trusts Supabase and nothing else.
 *
 * Root expires   2031-04-26
 * Intermediate   2033-10-21
 *
 * Both are pinned, so the chain still verifies if the server ever stops
 * sending the intermediate. To refresh them, download the certificate from
 * Supabase (Project Settings -> Database -> SSL configuration).
 */
export const SUPABASE_CA: string[] = [
  `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,
  `-----BEGIN CERTIFICATE-----

VCgJ
-----END CERTIFICATE-----`,
]
